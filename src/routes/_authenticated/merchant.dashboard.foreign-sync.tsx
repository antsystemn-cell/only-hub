...
function ForeignSyncPage() {
  const { user } = useAuth();
  const qc = useQueryClient();
  const [merchantId, setMerchantId] = useState<string | null>(null);

  // Resolve current merchant via merchant_users → first.
  useQuery({
    queryKey: ["merchant-of-user", user?.id],
    enabled: !!user?.id,
    queryFn: async () => {
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div>
          <h1 className="text-2xl font-bold">Гадаад эх сурвалжийн sync</h1>
        ...
        <Card className="p-4">
          <h2 className="mb-3 font-semibold">Бүтээгдэхүүний sync төлөв</h2>
              ...
              <tbody>
                {(productsQuery.data ?? []).map((p: any) => (
                  <tr key={p.id} className="border-t">
                    ...
                    <td className="py-2 pr-3">
                      {p.source_sync_status === "OK" && <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100">OK</Badge>}
                      {p.source_sync_status === "FAILED" && <Badge variant="destructive">FAILED</Badge>}
                      {p.source_sync_status === "NEEDS_REVIEW" && (
                        <Badge className="bg-orange-100 text-orange-700 hover:bg-orange-100">Хяналт</Badge>
                      )}
                      {p.low_stock_warning && (
                        <Badge className="ml-1 bg-amber-100 text-amber-700 hover:bg-amber-100">Үлдэгдэл бага</Badge>
                      )}
                    </td>
