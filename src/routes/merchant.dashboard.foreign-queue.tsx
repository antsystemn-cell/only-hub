  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: roles } = await supabase
        ...
        .limit(1);
      if (roles?.[0]?.merchant_id) setMerchantId(roles[0].merchant_id);
    })();
  }, []);
                  ...
                  >
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div>
                        ...
                        <div>{o.guest_name ?? "—"} · {o.phone ?? "—"}</div>
                        {r.selected_size_label && (
                          <div className="mt-1 text-xs">Хэмжээ: <b>{r.selected_size_label}</b></div>
                        )}
                        {r.source_variant_id && (
                          <div className="text-xs text-muted-foreground">SKU: {r.source_variant_id}</div>
                        )}
                      </div>
                      <div className="text-xs">
                        ...
                        <div>Хэрэглэгчээс: <b>{fmtMnt(Number(r.customer_paid_price_mnt ?? 0))}</b></div>
                      </div>
                    </div>
