...
interface StoreViewProps {
  ...
  /** When true, treat as index-page render (don't defer to <Outlet />). */
  forceIndex?: boolean;
}

export function StoreView({ merchantSlug, initialCategory, forceIndex }: StoreViewProps) {
  const pathname = useRouterState({ select: (s) => s.location.pathname });
  const isStoreIndex =
    forceIndex ||
    pathname === `/store/${merchantSlug}` ||
    pathname === `/store/${merchantSlug}/`;
    ...
    queryFn: async () =>
      (
        await supabase
          .from("merchants")
          .select(
            "id,name,slug,logo_url,description,is_active,approval_status,followers_count,created_at,updated_at",
          )
  ...
  const { data: products = [] } = useQuery({
    ...
    enabled: !!merchant?.id,
    queryFn: async () =>
      (await supabase.from("products").select("*").eq("merchant_id", merchant!.id).eq("is_active", true)).data ?? [],
  });
  ...
  const { data: categories = [] } = useQuery({
    ...
    enabled: !!merchant?.id,
    queryFn: async () =>
      (await supabase.from("categories").select("*").eq("merchant_id", merchant!.id).order("position")).data ?? [],
  });
  ...
  const activeCategoryObj = (categories as any[]).find(
    (c) => c.slug === activeCategory || c.name === activeCategory,
  );

  const isOrdersMerchant = merchantSlug === "orders";
  const catBase = isOrdersMerchant ? "/orders" : `/store/${merchantSlug}`;
  ...
  return (
    <div className="min-h-screen bg-[#fafafa]">
      ...
      <div className="container mx-auto px-4 py-6 sm:py-8">
        <div className="flex items-center gap-4 rounded-2xl border border-border/60 bg-white p-4 shadow-sm sm:p-5">
          {merchant.logo_url ? (
            <img
              ...
              alt={merchant.name}
              className="h-14 w-14 shrink-0 rounded-2xl object-cover ring-2 ring-orange-100 sm:h-16 sm:w-16"
            />
          ) : (
          ...
          <div className="min-w-0">
            <h1 className="truncate text-xl font-bold sm:text-2xl">
              {activeCategoryObj ? activeCategoryObj.name : merchant.name}
            </h1>
            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground sm:text-sm">
              <span>
                <span className="font-semibold text-foreground">{filteredProducts.length}</span> бараа
              </span>
              {!activeCategoryObj && (
                <>
                  <span>•</span>
                  <span>
                    <span className="font-semibold text-foreground">
                      {(merchant as any).followers_count ?? 0}
                    </span>{" "}
                    дагагч
                  </span>
                </>
            ...
            {!activeCategoryObj && merchant.description && (
        ...
        {banners.length > 0 && !activeCategoryObj && (
                ...
                >
                  ...
                  <div className="absolute inset-0 flex items-center bg-gradient-to-r from-black/60 to-transparent p-6 sm:p-8">
                    <div>
                      ...
                      {banner.button_text && (
                        <Button
                          className="mt-3 rounded-full bg-orange-500 hover:bg-orange-600"
                          asChild
                        >
                ...
                {(banners as any[]).map((_, i) => (
                  <button
                    ...
                    className={`h-1.5 rounded-full transition-all ${
                      i === activeBanner ? "w-5 bg-orange-500" : "w-1.5 bg-white/60"
                    }`}
                  />
                ))}
              </div>
        ...
        {categories.length > 0 && (
          <div className="mt-5 flex gap-2 overflow-x-auto pb-2 scrollbar-none">
            ...
            {(categories as any[]).map((cat) => {
              ...
              const href = cat.slug ? `${catBase}/${cat.slug}` : catBase;
              return (
                <Link key={cat.id} to={href as any}>
                  <Button
                    size="sm"
        ...
        <div className="mt-6 grid grid-cols-2 gap-2.5 sm:gap-3 md:grid-cols-3 lg:grid-cols-4">
          ...
          {filteredProducts.length === 0 && (
            <p className="col-span-full py-10 text-center text-muted-foreground">Бараа алга</p>
          )}
        </div>
      </div>
      <SiteFooter />
